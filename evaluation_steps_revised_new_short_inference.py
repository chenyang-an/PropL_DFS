...
def shorten_prompts(string):
    string_list = string.split('\n')
    reverse_string_list = string_list[::-1]

    for idx, line in enumerate(reverse_string_list):
        if 'no solution' in line:
            break
    pattern_1 = r'return to state (\d+)'
    match_1 = re.search(pattern_1, line)
    return_to_state_number = int(match_1.group(1))
    for idx, line in string_list:
        if f'state_{return_to_state_number}':
            break

    new_string_list = string_list[idx:]
