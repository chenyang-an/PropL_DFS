...
class DFS:
    ...
    def revise_output_list(self, output_text):
        #print(f'the output text for Round {self.round} is {output_text}')
    ...
    def collect_inference_result(self, outputs):
        for idx, output_list in tqdm(enumerate(outputs), total=len(outputs), desc=f"Processing LLM output for Round {self.round}"):
            ...
            for i in range(0, self.num_sampled_tactics):
                actual_output = output_list.outputs[i].text
                #print('actual output is:', actual_output)
                output_tactic = self.revise_output_list(actual_output)
            ...
            if len(tactic_list_at_top_per_key) != 0:
                ...
                try:
                    if len(entered_tactic)==2:
                        ...
                        #print(f'key is {key}, get proof after apply tactic is: {self.theorem_object_dict[key].get_current_lean_proof()}')
